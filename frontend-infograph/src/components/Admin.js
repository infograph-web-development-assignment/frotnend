...
class Admin extends React.Component {
  ...
  SaveAdmin = async (data) => {
    ...
    const addAdmin = await axios.post(`${this.state.server}/saveAdmin`, data);
  };
  ...
  loggingIn = (event) => {
    ...
    this.state.allAdmins.map((element) => {
      if (
        this.state.loginPassword === element.password &&
        this.state.loginUsername === element.username &&
  ...
  render() {
    return (
      <div
        style={{
          backgroundImage:
            "url(https://i1.wp.com/www.alphr.com/wp-content/uploads/2017/10/what_is_seed_funding-scaled.jpg?zoom=2&resize=738%2C320&ssl=1",
        }}>
        <ToolBar />
        ...
        {this.state.isAuthenticated && <AuthenticatedAdmin />}
        <Footer />
      </div>
    );
  }
